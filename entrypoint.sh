...
${STEAMCMDDIR}/steamcmd.sh +force_install_dir ${STEAMAPPDIR} +login anonymous +app_update ${STEAMAPPID} ${STEAMAPPBRANCH:+-beta "$STEAMAPPBRANCH"} validate +quit
...
${STEAMAPPDIR}/start-server.sh -servername ${SERVER_NAME} -adminpassword ${ADMIN_PASSWORD} -steamport1 ${STEAM_PORT_A} -steamport2 ${STEAM_PORT_B} -beta
